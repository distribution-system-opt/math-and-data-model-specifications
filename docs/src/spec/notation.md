...
```math
\ell ij \in \mathcal{T}^{L\rightarrow} \subset \mathcal{L}\times\mathcal{I}\times\mathcal{I},
\qquad
...
```math
d i \in \mathcal{C}^{D}\subset\mathcal{D}\times\mathcal{I},
\quad
g i \in \mathcal{C}^{G},
\quad
h i \in \mathcal{C}^{H},
...
Two four-terminal buses $\mathcal{I}=\{\texttt{A},\texttt{B}\}$ joined by one line
