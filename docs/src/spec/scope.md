...
- **Conductors:** 1- to 4-wire lines with full mutual coupling; explicit neutral and earth
  ...
  !!! danger "Amrit"
      Are we sticking to a maximum of 4 wires, or are we supporting general n-wire
      lines? Need to decide.
...
- The default objective is linear generation/dispatch cost; quadratic cost terms are not
  ...
  !!! danger "Amrit"
      Adding a quadratic cost term is trivial. Why are we excluding it? For the current
      cases we could simply include it with the coefficient set to 0.0.
