...
primary and secondary share no conductor, only magnetic flux.
Because the winding topologies differ qualitatively, each
...
idealised winding pair plus three loss components. Symbols are
...
object instead carries a `windings` array (each with `bus`, `terminal_map`, `v_ref`,
...
line-to-line for a delta winding (whose `v_ref` is the line-to-line coil voltage, so the
